...
def infer_tiled(model, lr_rgb, scale=2, tile=512, overlap=32, amp=True, device="cuda"):
    ...
    For models with a global bicubic residual, only the learned correction is
    blended across tiles; the bicubic base is computed once on the full image.
    This avoids tile-boundary interpolation artifacts.
    """
    model.eval()
    if device == "cuda" and not torch.cuda.is_available():
        device = "cpu"

    # Blend the correction only; add the full-image bicubic base at the end.
    global_residual = getattr(model, "global_residual", False)
    if global_residual:
        model.global_residual = False
    ...
    try:
        ...
        for y in range(0, H, step):
            for x in range(0, W, step):
                ...
                sr_tile = sr_tile.squeeze(0).float().cpu().numpy().transpose(1,2,0)
        ...
        if global_residual:
            model.global_residual = True

    w_acc = np.maximum(w_acc, 1e-6)
    sr = sr_acc / w_acc[..., None]

    if global_residual:
        base = cv2.resize(lr_rgb.astype(np.float32) / 255.0, (out_w, out_h),
                          interpolation=cv2.INTER_CUBIC)
        sr = sr + base

    sr_u8 = np.clip(np.clip(sr, 0, 1) * 255.0 + 0.5, 0, 255).astype(np.uint8)
    return sr_u8
